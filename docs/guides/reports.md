...
snapshot of your collection" shape as the other six.
...
**Set Workshop → 7 Reports** offers all six directly (two ask which set(s) first). It reuses the same export screen as
everywhere else in the app (**X** on Missing Parts, Owned Parts, etc.) — pick **P** for the printable report, or any of
the other formats. Saved files and download links/QR codes work exactly as described in
[Export and import](export-import.md#download-it-with-a-qr-code).

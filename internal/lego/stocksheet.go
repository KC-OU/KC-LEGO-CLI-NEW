...
func (d *DB) StockSheet(ctx context.Context, rb *Client, setNum string) (title string, lines []StockSheetLine, err error) {
	...
	if c, cerr := d.LastCheck(setNum); cerr == nil && c != nil {
		for _, l := range c.Lines {
			lines = append(lines, StockSheetLine{PartNum: l.PartNum, PartName: l.PartName, Category: l.Category, ColorName: l.ColorName, Expected: l.Need})
		}
	...
	for _, it := range inv.Items {
		...
		seen[k] = len(lines)
		lines = append(lines, StockSheetLine{PartNum: it.PartNum, PartName: it.PartName, ColorName: it.ColorName, Expected: it.Qty})
	}
...
{{range .Lines}}<tr><td class="box"><span></span></td><td>{{.PartNum}}</td><td>{{.ColorName}}</td><td>{{.PartName}}</td><td class="n">{{.Expected}}</td><td class="n"></td></tr>
{{end}}</table>
